...
def build_dummy_tf_classifier(input_shape: tuple, activation: str = None, optimizer: str = "adam"):
    ...
    if activation == "softmax":
        ...
        logger.warning(f"Activation was set to {activation} which is not supported.")
...
def build_dense_model(input_shape: tuple, num_hidden: int = 64, optimizer: str = "adam"):
    ...
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    logger.debug("Dense model built successfully")
    return model
...
def build_cnn_model(input_shape: tuple,
    ...
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    logger.debug("CNN model built successfully")
    return model
...
def build_lstm_model(input_shape: tuple,
    ...
    optimizer = keras.optimizers.Adam(learning_rate=learning_rate)
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    logger.debug("LSTM model built successfully")
    return model
...
def build_bi_lstm_model(input_shape: tuple,
    ...
    optimizer = keras.optimizers.Adam(learning_rate=learning_rate)
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    logger.debug("Bidirectional LSTM model built successfully")
    return model
